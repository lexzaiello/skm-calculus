...
def toStringImpl (e : LExpr) : String :=
  match e with
    ...
    | .lam t body       => s!"λ (_:{t.toStringImpl}) => {body.toStringImpl}"
    | .fall t body      => s!"∀ (_:{t.toStringImpl}), {body.toStringImpl}"
    | .m                => "M"
    | .k                => "K"
    | .s                => "S"
    | .ty n             => s!"Type {n}"
...
partial def type_of (e : LExpr) : LExpr :=
  match e with
    | .fall t body   => type_of body
    | .lam t body => .fall t (type_of body)
    | .ty n => .ty n.succ
...
partial def lift (e : LExpr) : LExpr :=
  ...
  let e' := match e with
    ...
    | .lam t (.call lhs rhs) =>
      let lhs' := lift lhs |> LExpr.lam t
      let rhs' := lift rhs |> LExpr.lam t

      lift (.call (.call .s lhs') rhs')
    | .lam t (.var 0) =>
      (.call (.call .s (.ty (.arrow  .k) .k)
    | .lam (.lam x') =>
      let body' := lift (.lam x')
...
def t_k' : Expr := (.lam (.lam (.call (.call .m (.var 1)) (.call (.call .m (.var 0)) (.var 1)))))
  |> lift
  |> to_sk_unsafe
