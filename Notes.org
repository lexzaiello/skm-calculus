...
* Representing S type with a well-defined fixpoint

One potential approach to make the type of S uses tuples.
Using α β γ, we can form (α, (β, γ))

Then, we can easily form α ~> (β ~> γ):

Assume we have access to fst and snd for projection:
fst (α, β) = α

We can just recursively prepend fst in each tuple with ~>

types := (α, (β, γ))

S (S (K ~>) fst) (S snd (K ~>)
S (S (K ~>) fst) (S snd (K ~>) (α, (β, γ))
= (~> α) ((β, γ) ~>) = ~> α (~> β γ) = α ~> β ~> γ
