...
Since we don't have reflection, we need to know the type of ~>.

M lifts something to being a type, but the argument could just be any expr.
...
~> is actually →, since S or K logic inside a type does dependency.

No need for ~>.

α → β is not of type Type.

α → β : M Type

We shouldn't be able to apply anything to α → β.

M is the only thing we can use to make types.

→ : M (Type → Type → M Type)

K : M (S Type ? ? (S Type ? ? (K Type (M (Type → Type → (M Type)) ~>) M) (S (K (S (S (K ~>) M)) (S (S (K S) (S (K K) ~>)) <~#))))

We can only normalize in quotation at the type-checker level, but not within the program.
...
This isn't that hard to make.

But we need to add the Type → Type stuff.

S (S (K S) (S (K K) →)) (←) α

K : S (K (→ Type)) (S (K (S (K (→ Type)))) (S (S (K S) (S (K K) ->)) <-)))
K α : Type → (S (K (→ Type)) (S (K (→ α)) (← α))
S β : Type → (α → (β → α))
S α β : α → β → α
    ...
    (K ((Type → Type) → Type → (M Type)) Type
     ...
     (S Type (Type → (M Type)) (M Type)
       (S Type (Type → Type → (M Type)) ((Type → (M Type)) → Type → (M Type))
         (K ((Type → Type → (M Type)) → (Type → (M Type)) → Type → (M Type)) Type
...
S α β γ : (M (S α ? ? (K (~> α)) (S ? ? ? (S (K ~>) β) γ)) → (M (S α ? ? (K (α ~>)) β) → (M α) → 

Type of S seems mildly impossible to produce by hand.
We just sant to restrict x such that was can plug in (z : α) and y z.
We want to restrict y such that it can accept z.r

HOWEVER, we should be able to plug in → types, as well.

Possible ways to unify → and dependent S types:
- → is sugar for S
- Custom → evaluation rules (could be helpful for making S type easier)
- clever S type.

Our current function application judgment rule says that applying the argument to f should produce an arrow type.
However, arrow types are sugar for something else?

This shouldn't be the case.
→ shouldn't be sugar and it shouldn't have eval rules.
It's just syntax.

We could say that a function application is valid if the left hand side IS an arrow type, or if applying the argument produces an arrow type, but this is ugly.

We don't want special cases.

Dependent and nondependent functions should be unified.
So → needs an eval rule.

(α → β) arg = (α → β)

But this is tricky again because we can't express the type of →.

Also note that in order to type-check a K call, we have to type-check a bunch of other calls.

Another potential idea: since quotation doesn't normalize, we can literally use untyped combinators in the M call (since M is just quotation).

e.g.,

Two possible routes for quotation (with untyped combinators in quotation):
- No normalization
- Normalization, but it's fine?

Is anything stopping us from injecting M(omega) into something then forcing us to evaluate it?

Obviously, here we would be forced to evaluate omega.
That's bad.

We must restrict what can be run inside M.
Potentially no normalization at all.

Pattern matching?

We can't handle beta-eq types, then, but we should be able to "transition" between types.

We can insert into M, though, and this doesn't break anything.

We can't copy α and β inside M.

S (K M) α = M α

We really just need to copy α and β into M.
The structure of M is kind of irrelevant.

Another thought: we allow untyped combinators in M and evaluation in M,
but interpretation is up to the runtime.

Everything injected into the type of K should be inert. There should be no normalization there.

Since α and β are types, they are also quoted. They are inert.

This doesn't really solve the → question though.
Well, it does, since the computation is happening outside the type.

K α β : (some ordering of α β α)

K α β : M (α → β → α)

or

K α β : M (k_type_untyped α β)

K : S (K (S (K M))) k_untyped

K α : (S (K M)) (k_untyped α)
K α β : M (K_untyped α β)

But here we have to know the type of M.

M is untyped.

We can potentially extend M to be typed.
...
Ok problem is though

We can probably change the type of arrow to not include Type as the output.
Or just not include M type.

M (α → β) : Type

α → β is kinda just garbage

It has no real type, since there's no eval rule.
It's void?

→ : Type → Type → 

What does → actually signify?

→ doesn't do anything so it's high-key a garbage value.

→ is really only valid in the context of M.

α → β is just syntax. you can't supply any argument to it to get something.

We want to remove Type from the type of → though.

α → β : Syntax.

Syntax = ad-hoc type for α → β?

Or can we say α → β : M (α → β)

M ? Type - this requires knowing the type of Type.

M Type : Type

In this context, Type is just syntax.

M : Syntax -> Type

We don't want to add a new construct, like operators.

But we would like M to be typed.

To get the type of an app, we always apply the argument to the type of the lhs.

M : ← Type

However, this implies that M's type is not actually a type.

M α : α → Type

This is actually probably fine.

M is just saying we are making this inert and of type tyep.

But we probably want:

But then we need to know the type of α → Type

→ doesn't really do anything. It's kinda just "syntax."
...
Now we can't plug in (← Type) into K.

Also having an explicit type argument is really annoying.

K (M (← Type))

OR what if M is just like anonymous :?

It's introducing something as a type.

It would be really cool if we could just add arbitrary syntax.

T (x : Syntax) : T (Type)



But for the quotation function, what is its type?
...
ANOTHER FUCKING BRAIN BLAST.

What if K and S are just Syntax?

And we have to MAKE them into functions?
...
We just need some way to "relate" the K syntax to the actual K.

This is what M does, really.

M : * → Syntax

Is → a type constructor or nah?

No, → is just syntax.

→ : Syntax → Syntax → Syntax

But we're using → as a type constructor here.

→ : T (Syntax → Syntax → Syntax)

Everything has type syntax?

Everything can be coerced to syntax?

What if we have a thing "maker" that makes it both syntax and a specific type simultaneously?

M e : Syntax

e can be literally anything.

This gets back to the polymorphism problem.

What can't e be?

Wait remember.

M e is not point-free. M e : Syntax, but M does not have a type itself.

We can't quote aribtrary data.
...
Can we get around separating M and T?

Only if:

M e : Type

But then we can't do our fancy insertion stuff in the type of K.

K α β : (M (k_type_maker α β))

Since we need to copy α and β into M.

But, like I said M e : Type

e has to be known.

Or we can add the type argument to M, but I don't like that.

M α : α → Type

We don't need to quote, necessarily.

M : ← Type

In fact we probably ought not quote.

I really really really like the syntax idea though.

T : T (Syntax → Type)

This is confusing, because clearly → is a type constructor.

If → is a type constructor, and T is a type constructor, then what is a type constructor?

→ : Syntax

Syntax and type-checking are closely-related, but not the same.
We can have multiple type contexts for the same syntax, ideally.

T → clearly says that → is a type.

We would like to unify dependent and nondependent types with T.

T is the only type constructor.

But T doesn't say how we "form" the type from syntax.

Dumb solution:

→ is syntax in the context of T, but in other contexts, it might be something different.

So T takes any syntax.

T (e) is a theorem, but we have no way to prove it.

I really like this formation rule idea. It could say how we can turn the K x y syntax into logic.

T : T (Syntax → Type)

K is a formation rule for α → β → α.

S is a formation rule for etc...

We don't want different type formations to override each other in the global context.
Type formation rules are hypotheses essentially.

K formation rule:

in some context, the syntax K α β applied to some function f produces the type α → β → α

K has a constructor that takes in α and β.

T : Syntax → Type

T K : Type

Our stratification thing makes stuff easier.

This is recursive, bozo.

Everything has type Syntax.

We instantiate K and S from the syntax.

K and S are emergent.
...
We need a concept to represent this, since → doesn't really have a type.

We could say:

→ α β : Syntax

I really like the syntax / type distinction.
...
This way we can remove → entirely from our AST.

Except now → needs to be an AST element since it has a type formation rule.

I would really like if we could extract out this "formation rule" idea.

T is "type formation rule." It is a term that represents a proof that type t can be formed with the type s.

T : Syntax → Syntax → Type
T K (α → β → α) : Type

T is the assertion "this is the type of this thing in this context"

T (→) (Type → Type → Type)

T → (Syntax → Syntax → Syntax)

This gives us inductive types, I'm pretty sure.

All expressions start with the context T K ... and T S ....

→ is just syntax now, though, so that's nice.

We can still leave K and S fully typed, though.

K : 

The goal is T K (

Also note that Type is just syntax.

T is our only type constructor.

You give it the syntax, the syntax for its type, and it produces a type.

T (→) (Type → Type → Type)

We can't express the type of K with T, though.

We NEED to be able to do some logic in types.

T K (...) is the assumption that K has that type.

But this isn't saying K : Syntax.

It's saying under that context, K : ...

EVERYTHING is syntax by default.

T T (Syntax → Syntax → Type)

Or we use M to make syntax?
But then we need to know the type of M...

We certainly need K and S to compute.

So we can't bootstrap those.

So the assumption T K ... is implicit. They're axioms.

Now, the type of K:

T (K α β) (α → β → α)

We can make a K type maker?

Does Type exist either?

α and β just have to be syntax.

(T Me Human : Type)

T (K (T Me Human) (T Me Human)) (...)

arrow_maker = (S (K (-> Type))
      (S (K (S (K (-> Type))))
        (S (S (K S) (S (K K) ->))
              <-)))

arrow_maker α β = α → β → α
...
What is our function application judgment rule now?

Function application is application of syntax nodes.

K α β are all syntax.

We need some proof in our context that the left hand side node produces what?

This is where we need specific interpretation.
We can't just leave it agnostic to the implementation.
Or can we?

K is the context: well-typed, for any α and β, so long as they are types.
...
There is a rule in our context that says x is well-typed.

So we're applying x to α.

But what does applying x to α even do?
Nothing.

T should have some way to "reject."

Or, rather, (T x Syntax) is just the example.

T must have some way to accept multiple possible values.

Also, we shouldn't have to specify every value.
We should be able to predicate it.

We can probably build this later using T.

K : (T α Type) → (T β Type) → (T x α) → (T y β) → (T (K α β x y) α)

Technically, our T thing probably doesn't need the first syntax argument.

K : α@(T Type) → β@(T Type) → (T α) → (T β) → (T α)

Even in a context, T with a symbol can override other things. We don't want this.

We want some term to produce (T whatever), so we say that we can construct (T whatever) using that term ("constructor")

→ is just syntax, though, remember.

Maybe we have 2 type constructors?
→ and T?

We can't really encode → now, since we don't have the syntax #1 parameter.

T : T (Syntax → Type)

M e : Syntax

Clearly, → has some special meaning, just like T.

Since we intend to use → within types, it should work on syntax.
Or should it work on types?

We normalize inside T, since it's not syntax yet.

→ : T (Type → Type → Syntax)

We can probably derive → here, ngl. But it would be mad complicated.

T : Syntax → Type

K type, eta expanded

K α β : T (T (M α) → T (M β) → T (M α))

T isn't the only type constructor.
→ is as well. T is like "base type."

We could change → such that:

→ : Type → Type → Type

K α β  : α → β → α

A dependent function is just a function that creates a new type context for the variable.

T is the only type constructor and it creates a type from syntax.

How do we do dependent elimination then?

Pattern matching.

→ is actually just syntax.

We need different kinds of syntax nodes though.

It's not all just syntax.

It's Syntax or Syntax → Syntax → Syntax.

But these are both Syntax?

#+BEGIN_SRC lean
inductive Syntax where
  | leaf : Syntax
  | branch : Syntax → Syntax → Syntax
#+END_SRC

I don't understand what this T and M stuff was for.
Just unnecessary abstraction.

Answer: it allows us to remove sorts (supposedly).

Anything is syntax, but → is a type → type → type

→ : Type → Type → Type

T : Syntax → Type

T is like "base type introduction"
and → is like makine a newtype

Except we don't have a way to introduce dependent types!

Perhaps S is actually the dependent type introducer?

K is the nondependent type introducer?

We either need to have → a syntax or a term.
Advantages to syntax: very simple.

We have an easy inductive definition of Type.

K α β : (T (M α)) → (T (M β)) → (T (M α))

Except since we need to take in α and β, we can't use the type of K as an actual type.

K : some S expression resolving to →

but then we can't use the type of K anywhere.

We need to be able to construct Type more flexibly than with →.

K : T (M (some S expression))

Type-checking algorithm:

Run "elab", then try plugging in.

Dependent S now.

S α β γ (x : S (K (→ α)) (S (S (K →) β) γ) (y : S (K (→ α)) β) (z : α), γ z (y z)

S : ∀ (α : Type) (β : α → Type) (γ : S (K (→ α)) (S (S (K →) β) Type)), γ z (y z)

S : S (K (→ Type)

New idea with syntax:

Syntax nodes can take in other syntax nodes, transform them in certain ways.

K : Syntax → Syntax → Syntax → Syntax → Syntax

I don't really like this concept of just interpreting syntax inside a type.

T obviously creates a type.

We can have "quoted" versions of K and S.

M K : Syntax → Syntax → Syntax
...
α and β have to be quoted.
But also, α and β are types.

→ : Type → Type → Syntax

Note that T may include an expression that takes in more syntax.

T (M K)

We should be able to do T (M K) α β = T ((M K) α β)

T ((M K) α β) : Type, but

T (M K) : T (Syntax → Syntax → Type)
...
So T must be polymorphic.

Or we can say:

T t (e : t) : t

T (T (Type → Type → Syntax)) (→) : (T (Type → Type → Syntax))

→ α β : T Syntax

Why do we need the e : t parameter?

T (T (Type → Type → Syntax)) : T (T (Type → Type → Syntax) → T (Type → Type → Syntax))
...
We can pattern match on M K.

M K α β is just syntax.

We can "interpret" (M K α β) as (α → β → α) via pattern matching.

K : T (M K)

T with type parameter is recursive. Don't like this.

e.g., T Syntax

Need to know the type of Syntax, which is T Syntax.

This just recurses forever.

Type of Syntax is T Syntax.

is the type argument redundant?

T of anything is a type.

But now we can't feed thigns into T.

T Syntax takes no arguments.

Types are monoids.

T e : Type.

Not everything must be of a type of type type.

T just lifts something to type type.
