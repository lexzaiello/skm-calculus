...
$R(t)$ can be similarly extended to include the S combinator and obviously reducible expressions:
...
\{ S \mid \forall \alpha : T_{n},\ \beta : T_{m},\ \gamma : T_{o},\ T_{x} = (\forall x : \alpha, y : \beta.\gamma),\ T_{y} = (\forall x : \alpha.\alpha),\ T_{z} = \alpha, \notag \\
  ...
  arg_{1} \in R(T_{x}) \land arg_{2} \in R(T_{y}) \land arg_{3} \in R(T_{z}) \} \union \notag \\
\{ fall |  \}
\end{gather*}
#+END_EXPORT
