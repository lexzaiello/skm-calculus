...
\begin{proof}
Assume (1) that for all $c \in B, \exists!\ c' \in M_{c}, c' = M_{c} c$. Assume (2) that for all $\{t_{1}, t_{2}, t\} \subset T, t = (t_{1} \rightarrow t_{2}), \exists!\ t' \in M_{t}, t' = M_{t} t$. Per \href{decomplemma:1}{above}, and induction on (1) there exists a mapping from every lambda expression to an SK combinator expression. It follows by induction on $e : t$, where e is well-typed per the \href{decomplemma:1}{inference rules} that all $t \in$ the simply-typed $T$ are in $M_{t}$.

\end{proof}
\section{Proof}
\label{sec:orgdb9f480}
\subsection{Strong Normalization of the Typed SK Combinators}
\label{sec:orgd5a7a7c}
\subsection{Strong Normalization of the STLC}
\label{sec:org5964f65}
\subsection{Encoding in Lean}
\label{sec:orgb43bf27}
