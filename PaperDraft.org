...
A natural interpretation of the combinators as typed functions results in the polymorphic typing:
...
#+NAME: decomplemma:1
\label{decomplemma:1}
* Decomposition of the Simply-Typed Lambda Calculus into Polymorphic Typed SK Combinators
...
I define a mapping (M_{t}) from the \rightarrow type constructor to \forall: $(\alpha \rightarrow \beta) \mapsto \forall x : \alpha.\beta$. I also assume the existence of a mapping (M_{c}) from the base types $B$ to arbitrary objects in my polymorphic SK combinator calculus. Type inference is trivially derived from the above inference rules: $\forall\ t \in B, \exists\ c : t, t', c : t \implies t' = M_{t} t \implies M_{c}\ c : t$.
...
It suffices to conclude that all well-typed expressions have well-typed counterparts in the polymorphic SK combinator calculus.
\end{proof}
#+END_EXPORT
